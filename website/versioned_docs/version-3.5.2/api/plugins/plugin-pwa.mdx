...
Having a service worker installed is not enough to make your application a PWA. You'll need to at least include a [Web App Manifest](https://developer.mozilla.org/Web/Manifest) and have the correct tags in `<head>` ([Options > pwaHead](#pwahead)).
