...
Permits to style the blog XML feeds so that browsers render them nicely with [XSLT](https://developer.mozilla.org/Web/XSLT).
...
| `last_update` | `FrontMatterLastUpdate` | `undefined` | Allows overriding the last update author/date. Date can be any [parsable date string](https://developer.mozilla.org/Web/JavaScript/Reference/Global_Objects/Date/parse). |
