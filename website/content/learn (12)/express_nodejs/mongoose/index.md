...
This article briefly introduces databases, and how to use them with Node/Express apps. It then goes on to show how we can use [Mongoose](https://mongoosejs.com/) to provide database access for the [LocalLibrary](/content/Learn/Server-side/Express_Nodejs/Tutorial_local_library_website) website. It explains how object schema and models are declared, the main field types, and basic validation. It also briefly shows a few of the main ways in which you can access model data.
...
<table>
  <tbody>
    <tr>
      ...
      <td>
        <a href="/content/Learn/Server-side/Express_Nodejs/skeleton_website">Express Tutorial Part 2: Creating a skeleton website</a>
      </td>
    </tr>
    <tr>
...
The rest of this tutorial shows how to define and access the Mongoose schema and models for the [LocalLibrary website](/content/Learn/Server-side/Express_Nodejs/Tutorial_local_library_website) example.
...
Historically JavaScript relied heavily on passing [callback functions](/content/Learn/JavaScript/Asynchronous/Introducing) to asynchronous methods to handle the success and error cases.
In modern JavaScript callbacks have largely been replaced by [Promises](/content/Web/JavaScript/Reference/Global_Objects/Promise).
...
There are two main ways you can use promises to run code when a promise is settled, and we highly recommend that you read [How to use promises](/content/Learn/JavaScript/Asynchronous/Promises) for a high level overview of both approaches.
In this tutorial, we'll primarily be using [`await`](/content/Web/JavaScript/Reference/Operators/await) to wait on promise completion within an [`async function`](/content/Web/JavaScript/Reference/Statements/async_function), because this leads to more readable and understandable asynchronous code.
...
`myFunction()` is an asynchronous function that is called within a [`try...catch`](/content/Web/JavaScript/Reference/Statements/try...catch) block.
...
This is done using the [`Promise.all()`](/content/Web/JavaScript/Reference/Global_Objects/Promise/all) method, which takes an iterable of promises as input and returns a single `Promise`.
...
> [!NOTE]
> Creation, update, deletion and querying of records are asynchronous operations that return a [promise](/content/Web/JavaScript/Reference/Global_Objects/Promise).
...
> In the [Express Tutorial Part 7: Deploying to Production](/content/Learn/Server-side/Express_Nodejs/deployment) tutorial we host both the application and database on [Railway](https://railway.app/), but we could equally well have used a database on [MongoDB Atlas](https://www.mongodb.com/products/platform/atlas-database).
...
Open a command prompt and navigate to the directory where you created your [skeleton Local Library website](/content/Learn/Server-side/Express_Nodejs/skeleton_website).
...
We'll show you how to do this more safely when [deploying to production](/content/Learn/Server-side/Express_Nodejs/deployment#database_configuration)!
