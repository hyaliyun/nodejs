...
Note that this replaces the placeholder asynchronous handler that we added in the [Express Tutorial Part 4: Routes and controllers](/content/Learn/Server-side/Express_Nodejs/routes#genre_controller) with a "normal" express route handler function.
...
> [!NOTE]
> Our validation uses `trim()` to ensure that whitespace is not accepted as a genre name. We also validate that the field is not empty on the client side by adding the [boolean attribute](/content/Glossary/Boolean/HTML) `required` to the field definition in the form:
>
> ```pug
> input#name.form-control(type='text', placeholder='Fantasy, Poetry etc.' name='name' required value=(undefined===genre ? '' : genre.name) )
> ```
...
1. Return to [Express Tutorial Part 6: Working with forms.](/content/Learn/Server-side/Express_Nodejs/forms)
2. Proceed to the next sub article of part 6: [Create Author form](/content/Learn/Server-side/Express_Nodejs/forms/Create_author_form).
