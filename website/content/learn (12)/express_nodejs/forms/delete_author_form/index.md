...
As discussed in the [form design](/content/Learn/Server-side/Express_Nodejs/forms#form_design) section, our strategy will be to only allow deletion of objects that are not referenced by other objects (in this case that means we won't allow an `Author` to be deleted if it is referenced by a `Book`).
...
- Proceed to the final subarticle of part 6: [Update Book form](/content/Learn/Server-side/Express_Nodejs/forms/Update_Book_form).
