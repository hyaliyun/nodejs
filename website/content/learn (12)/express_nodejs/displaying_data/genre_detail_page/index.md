...
Because the two requests do not depend on each other, we use `Promise.all()` to run the database queries in parallel (this same approach for running queries in parallel was demonstrated in the [home page](/content/Learn/Server-side/Express_Nodejs/Displaying_data/Home_page#controller)).
...
> [!NOTE]
> Errors passed to the `next` middleware function propagate through to our error handling code (this was set up when we [generated the app skeleton](/content/Learn/Server-side/Express_Nodejs/skeleton_website#app.js) - for more information see [Handling Errors](/content/Learn/Server-side/Express_Nodejs/Introduction#handling_errors)).
...
- Proceed to the next subarticle of part 5: [Book detail page](/content/Learn/Server-side/Express_Nodejs/Displaying_data/Book_detail_page).
