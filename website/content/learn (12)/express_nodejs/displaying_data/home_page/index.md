...
> Before continuing you may wish to re-read the [Mongoose primer](/content/Learn/Server-side/Express_Nodejs/mongoose#mongoose_primer) section on [searching for records](/content/Learn/Server-side/Express_Nodejs/mongoose#searching_for_records).

## Route

We created our index page routes in a [previous tutorial](/content/Learn/Server-side/Express_Nodejs/routes).
...
The code also requires "express-async-handler", which provides a wrapper to [catch exceptions thrown in route handler functions](/content/Learn/Server-side/Express_Nodejs/routes#handling_exceptions_in_route_functions).
...
Because the queries for document counts are independent of each other we use [`Promise.all()`](/content/Web/JavaScript/Reference/Global_Objects/Promise/all) to run them in parallel.
The method returns a new promise that we [`await`](/content/Web/JavaScript/Reference/Operators/await) for completion (execution pauses within _this function_ at `await`).
...
- Proceed to the next subarticle of part 5: [Book list page](/content/Learn/Server-side/Express_Nodejs/Displaying_data/Book_list_page).
