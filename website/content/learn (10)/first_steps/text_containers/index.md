  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        >) and some CSS notions on text styling (read <a href="/content/Learn/CSS/Styling_text/Fundamentals">fundamental text and font styling</a> and
          <a href="/content/Learn/CSS/Styling_text/Web_fonts">web fonts</a>).
      </td>
    </tr>
    <tr>
...
Since most of these characters are not part of Basic Latin Unicode block, it is recommended to specify your [document's character encoding](/content/Learn/HTML/Introduction_to_HTML/The_head_metadata_in_HTML#specifying_your_documents_character_encoding) and to serve it with appropriate [web fonts](/content/Learn/CSS/Styling_text/Web_fonts). Here is a basic template to use UTF-8 encoding and the [Latin Modern Math](/content/Web/MathML/Fonts#fonts_with_a_math_table) font:
...
> [!NOTE]
> An obvious difference is that the source code became much more verbose with MathML. Recall that this tutorial is about learning the language but in practice MathML content is generally not written manually. See the [Authoring MathML](/content/Web/MathML/Authoring) page for more information.
...
> [!WARNING]
> Special [math fonts](/content/Web/MathML/Fonts) are generally required to make that stretching possible, the previous example relies on [web fonts](/content/Learn/CSS/Styling_text/Web_fonts).

## Summary

In this article, we have learnt about a few _token_ elements that are used as text containers as well as their different semantics, namely `<mi>` (identifier), `<mn>` (numbers), `<mo>` (operators), `<mtext>` (generic text). We have seen special Unicode characters that are commonly found in math formulas and given an overview of some observable behaviors of the `<mi>` and `<mo>` elements. In the next article, we will see how to rely on _token_ elements to build much complex expressions such as [fractions and roots](/content/Learn/MathML/First_steps/Fractions_and_roots).
...
- [The `<mi>` element](/content/Web/MathML/Element/mi)
- [The `<mn>` element](/content/Web/MathML/Element/mn)
- [The `<mo>` element](/content/Web/MathML/Element/mo)
- [The `<mtext>` element](/content/Web/MathML/Element/mtext)
