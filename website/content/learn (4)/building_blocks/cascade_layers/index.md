...
This lesson aims to introduce you to [cascade layers](/content/Web/CSS/@layer), a more advanced feature that builds on the fundamental concepts of the [CSS cascade](/content/Web/CSS/Cascade) and [CSS specificity](/content/Web/CSS/Specificity).
...
Cascade layers are most relevant when you're working with CSS from multiple sources when there are conflicting CSS selectors and competing specificities, or when you're considering using [`!important`](/content/Web/CSS/important).
  ...
  <tbody>
    <tr>
      ...
      <td>An idea of how CSS works, including cascade and specificity (study
        <a href="/content/Learn/CSS/First_steps">CSS first steps</a> and <a href="/content/Learn/CSS/Building_blocks/Cascade_and_inheritance">Cascade, specificity, and inheritance</a>).
      </td>
    </tr>
    <tr>
...
2. **Importance:** Sort rules based on whether they are normal or important. Important styles are those that have the [`!important`](/content/Web/CSS/important) flag set.
...
There are three [cascade origin types](/content/Web/CSS/Cascade#origin_types): user-agent stylesheets, user stylesheets, and author stylesheets. The browser sorts each declaration into six origin buckets by origin and importance. There are eight levels of precedence: the six origin buckets, properties that are transitioning, and properties that are animating. The order of precedence goes from normal user-agent styles, which have the lowest precedence, to styles within currently applied animations, to important user-agent styles, and then styles being transitioned, which have the highest precedence:
...
Within each of the six origin buckets, there can be multiple cascade layers. The [order of layer creation](/content/Web/CSS/@layer) matters a lot. It is the order of creation that sets the precedence order among layers within an origin.
...
- The [`@layer`](/content/Web/CSS/@layer) statement at-rule, declaring layers using `@layer` followed by the names of one or more layers. This creates named layers without assigning any styles to them.
- The `@layer` block at-rule, in which all styles within a block are added to a named or unnamed layer.
- The [`@import`](/content/Web/CSS/@import) rule with the `layer` keyword or `layer()` function, which assigns the contents of the imported file into that layer.
...
The [`@layer`](/content/Web/CSS/@layer) CSS at-rule is used to declare a cascade layer and to define the order of precedence when there are multiple cascade layers. The following at-rule declares three layers, in the order listed:
...
You can import styles and create layers based on specific conditions using [media queries](/content/Web/CSS/CSS_media_queries/Using_media_queries) and [feature queries](/content/Web/CSS/CSS_conditional_rules/Using_feature_queries). The following imports a style sheet into an `international` layer only if the browser supports `display: ruby`, and the file being imported is dependent on the width of the screen.
...
Inline styles are declared using the [`style` attribute](/content/Web/HTML/Global_attributes/style). Normal styles declared in this way will take precedence over normal styles found in the unlayered and layered style sheets (`firstLayer – A.css`, `secondLayer – B.css`, and `C.css`).
...
You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: The Cascade, Task 2](/content/Learn/CSS/Building_blocks/Cascade_tasks#task_2).

## Summary

If you understood most of this article, then well done — you're now familiar with the fundamental mechanics of CSS cascade layers. Next up, we'll look at [the box model](/content/Learn/CSS/Building_blocks/The_box_model) in detail.
