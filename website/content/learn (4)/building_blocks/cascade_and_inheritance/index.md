...
At some point, you will be working on a project and you will find that the CSS you thought should be applied to an element is not working. Often, the problem is that you create two rules that apply different values of the same property to the same element. [**Cascade**](/content/Web/CSS/Cascade) and the closely-related concept of [**specificity**](/content/Web/CSS/Specificity) are mechanisms that control which rule applies when there is such a conflict. The rule that's styling your element may not be the one you expect, so you need to understand how these mechanisms work.

Also significant here is the concept of [**inheritance**](/content/Web/CSS/Inheritance), which means that some CSS properties by default inherit values set on the current element's parent element and some don't. This can also cause some behavior that you might not expect.
...
[Specificity](/content/Web/CSS/Specificity) is the algorithm that the browser uses to decide which property value is applied to an element. If multiple style blocks have different selectors that configure the same property with different values and target the same element, specificity decides the property value that gets applied to the element. Specificity is basically a measure of how specific a selector's selection will be:
...
> [!NOTE]
> On MDN CSS property reference pages, you can find a technical information box called "Formal definition", which lists a number of data points about that property, including whether it is inherited or not. See the [color property Formal definition section](/content/Web/CSS/color#formal_definition) as an example.
...
- {{cssxref("initial")}}
  - : Sets the property value applied to a selected element to the [initial value](/content/Web/CSS/initial_value) of that property.
- {{cssxref("revert")}}
...
- {{cssxref("revert-layer")}}
  - : Resets the property value applied to a selected element to the value established in a previous [cascade layer](/content/Web/CSS/@layer).
- {{cssxref("unset")}}
  - : Resets the property to its natural value, which means that if the property is naturally inherited it acts like `inherit`, otherwise it acts like `initial`.

> [!NOTE]
> See [Origin types](/content/Web/CSS/Cascade#origin_types) for more information on each of these and how they work.
...
> [!NOTE]
> The universal selector ([`*`](/content/Web/CSS/Universal_selectors)), [combinators](/content/Learn/CSS/Building_blocks/Selectors/Combinators) (`+`, `>`, `~`, ' '), and specificity adjustment selector ([`:where()`](/content/Web/CSS/:where)) along with its parameters, have no effect on specificity.

The negation ([`:not()`](/content/Web/CSS/:not)), relational selector ([`:has()`](/content/Web/CSS/:has)), the matches-any ([`:is()`](/content/Web/CSS/:is)) pseudo-classes, and [CSS nesting](/content/Web/CSS/CSS_nesting/Nesting_and_specificity) themselves don't add to specificity, but their parameters or nested rules do. The specificity weight that each contributes to the specificity algorithm is the specificity weight of the selector in the parameter or nested rule with the greatest weight.

The following table shows a few isolated examples to get you in the mood. Try going through these, and make sure you understand why they have the specificity that we have given them. We've not covered selectors in detail yet, but you can find details of each selector on the MDN [selectors reference](/content/Web/CSS/CSS_selectors/Selectors_and_combinators).
...
Even though [cascade layers](/content/Web/CSS/@layer) is an advanced topic and you might not use this feature right away, it's important to understand how layers cascade.
...
Another advanced topic that you might not use right away but may need to understand in the future is {{CSSxRef("@scope")}}. This is an [at-rule](/content/Web/CSS/At-rule) that enables you to create a block of rules that will only apply to a specific subsection of the HTML on your page. For example, you can specify styles that will only apply to {{htmlelement("img")}} elements when they're nested inside elements that have a `feature` class:
...
> [!NOTE]
> Scoping proximity overrules source order but is itself overridden by other, higher-priority criteria such as [importance](/content/Web/CSS/important), [layers](/content/Learn/CSS/Building_blocks/Cascade_layers), and [specificity](/content/Web/CSS/Specificity).

## Test your skills!

You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: The Cascade](/content/Learn/CSS/Building_blocks/Cascade_tasks).

## Summary

If you understood most of this article, then well done — you've started getting familiar with the fundamental mechanics of CSS. Next up, we'll take a deeper look at [Cascade Layers](/content/Learn/CSS/Building_blocks/Cascade_layers).
