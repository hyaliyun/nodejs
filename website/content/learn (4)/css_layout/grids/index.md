...
[CSS grid layout](/content/Web/CSS/CSS_grid_layout) is a two-dimensional layout system for the web. It lets you organize content into rows and columns and offers many features to simplify the creation of complex layouts. This article will explain all you need to know to get started with grid layout.
...
In addition to creating grids using lengths and percentages, we can use [`fr`](/content/Web/CSS/flex_value). The `fr` unit represents one fraction of the available space in the grid container to flexibly size grid rows and columns.
...
> [!NOTE]
> You can also use the value `-1` to target the end column or row line, then count inwards from the end using negative values. Note also that lines count always from the edges of the explicit grid, not the [implicit grid](/content/Glossary/Grid).
...
It's possible to nest a grid within another grid, creating a ["subgrid"](/content/Web/CSS/CSS_grid_layout/Subgrid).
...
You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: Grid](/content/Learn/CSS/CSS_layout/Grid_skills).
...
- A [list of guides](/content/Web/CSS/CSS_grid_layout#guides) related to the CSS grid layout
- [Subgrid](/content/Web/CSS/CSS_grid_layout/Subgrid) guide
