...
- Start with learning the [critical rendering path](/content/Web/Performance/Critical_rendering_path) of the browser. Knowing this will help you understand how to improve the performance of the site.
- Using _resource hints_ such as [`rel=preconnect`](/content/Web/HTML/Attributes/rel/preconnect), [`rel=dns-prefetch`](/content/Web/HTML/Attributes/rel/dns-prefetch), [`rel=prefetch`](/content/Web/HTML/Attributes/rel/prefetch), [`rel=preload`](/content/Web/HTML/Attributes/rel/preload).
...
- [CSS](/content/Learn/Performance/CSS) performance factors
...
- Lazy loading parts of your application outside the viewport. If you do, have a backup plan for SEO (e.g., render full page for bot traffic); for example, by using the [`loading`](/content/Web/HTML/Element/img#loading) attribute on the {{HTMLElement("img")}} element
- It is also crucial to realize what is really important to your users. It might not be absolute timing, but [user perception](/content/Learn/Performance/Perceived_performance).

## Quick Wins

### CSS

Web performance is all about user experience and perceived performance. As we learned in the [critical rendering path](/content/Web/Performance/Critical_rendering_path) document, linking CSS with a traditional link tag with rel="stylesheet" is synchronous and blocks rendering. Optimize the rendering of your page by removing blocking CSS.
...
Avoid JavaScript blocking by using the [async](/content/Web/HTML/Element/script) or [defer](/content/Web/HTML/Element/script) attributes, or link JavaScript assets after the page's DOM elements. JavaScript only block rendering for elements that appear after the script tag in the DOM tree.
...
Within @font-face use font-display: swap. By using font display swap the browser will not block rendering and will use the backup system fonts that are defined. Optimize [font weight](/content/Web/CSS/font-weight) to match the web font as closely as possible.
...
- Define a [performance budget](/content/Web/Performance/Performance_budgets).

### APIs

- Gather user metrics using the [boomerang](https://github.com/akamai/boomerang) library.
- Or directly gather with [window.performance.timing](/content/Web/API/Performance/timing)
