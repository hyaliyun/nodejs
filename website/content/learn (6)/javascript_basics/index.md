...
It's outside the scope of this article—as a light introduction to JavaScript—to present the details of how the core JavaScript language is different from the tools listed above. You can learn more in MDN's [JavaScript learning area](/content/Learn/JavaScript), as well as in other parts of MDN.
...
> [!NOTE]
> Both of the features you used in this exercise are parts of the [Document Object Model (DOM) API](/content/Web/API/Document_Object_Model), which has the capability to manipulate documents.
...
> [!WARNING]
> In this article, try entering the example code lines into your JavaScript console to see what happens. For more details on JavaScript consoles, see [Discover browser developer tools](/content/Learn/Common_questions/Tools_and_setup/What_are_browser_developer_tools).

### Variables

{{Glossary("Variable", "Variables")}} are containers that store values. You start by declaring a variable with the [`let`](/content/Web/JavaScript/Reference/Statements/let) keyword, followed by the name you give to the variable:
...
You can name a variable nearly anything, but there are some restrictions. (See [this section about naming rules](/content/Web/JavaScript/Guide/Grammar_and_types#variables).) If you are unsure, you can [check your variable name](https://mothereff.in/js-variables) to see if it's valid.
...
Note that variables may hold values that have different [data types](/content/Web/JavaScript/Data_structures):
...
>9 - 3;<br />8 * 2; // multiply in JS is an asterisk<br />9 / 3;</code
    ...
    <tr>
      ...
      <td><a href="/content/Web/JavaScript/Reference/Operators/Strict_equality"><code>===</code></a></td>
      <td>
...
There are a lot more operators to explore, but this is enough for now. See [Expressions and operators](/content/Web/JavaScript/Reference/Operators) for a complete list.
...
Here we select the {{htmlelement("html")}} element. We then call its [`addEventListener()`](/content/Web/API/EventTarget/addEventListener) function, passing in the name of the event to listen to (`'click'`) and a function to run when the event happens.
...
- [Dynamic client-side scripting with JavaScript](/content/Learn/JavaScript)
