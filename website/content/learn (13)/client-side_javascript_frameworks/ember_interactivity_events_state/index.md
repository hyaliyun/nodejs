...
We can capture the [`keydown`](/content/Web/API/Element/keydown_event) event via the [on modifier](https://api.emberjs.com/ember/3.16/classes/Ember.Templates.helpers/methods/on?anchor=on), which is just Ember syntactic sugar around [`addEventListener`](/content/Web/API/EventTarget/addEventListener) and [`removeEventListener`](/content/Web/API/EventTarget/removeEventListener) (see [Introduction to events](/content/Learn/JavaScript/Building_blocks/Events) if needed).
...
And just like before, the `add()` function that will be called from the template gets annotated with the `@action` decorator to bind it to the class instance. We [spread our `todos` array](/content/Web/JavaScript/Reference/Operators/Spread_syntax) to add `newTodo` to it, which creates a new array and will trigger the reactivity system to update the UI.
...
One issue here is that our service is called `todos`, but the list of todos is also called `todos`, so currently we would access the data using `this.todos.todos`. This is not intuitive, so we'll add a [getter](/content/Web/JavaScript/Reference/Functions/get) to the `todos` service called `all`, which will represent all todos.
...
With a dynamic `#each` block (which is basically syntactic sugar over the top of JavaScript's [`forEach()`](/content/Web/JavaScript/Reference/Global_Objects/Array/forEach)) that creates a `<Todo />` component for each todo available in the list of todos returned by the service's `all()` getter:
