    ...
    <tr>
      ...
      <td>
        <p>
          ...
          <a href="/content/Learn/JavaScript">JavaScript</a> languages, and
...
We'll focus on some accessibility issues involving focus management. To do so, we'll utilize some techniques for accessing DOM nodes and executing methods like [`focus()`](/content/Web/API/HTMLElement/focus) and [`select()`](/content/Web/API/HTMLInputElement/select). We will also see how to declare and clean up event listeners on DOM elements.
...
In this case we are using the [`map()`](/content/Web/JavaScript/Reference/Global_Objects/Array/map) method, which returns a new array with the results of executing the provided function for each item. The function returns a copy of each to-do using [spread syntax](/content/Web/JavaScript/Reference/Operators/Spread_syntax) and overwrites the property of the completed value accordingly. This solution has the added benefit of returning a new array with new objects, completely avoiding mutating the original `todos` array.
...
To implement all these features, we'll need programmatic access to DOM nodes to run functions like [`focus()`](/content/Web/API/HTMLElement/focus) and [`select()`](/content/Web/API/HTMLInputElement/select). We will also have to use [`addEventListener()`](/content/Web/API/EventTarget/addEventListener) and [`removeEventListener()`](/content/Web/API/EventTarget/removeEventListener) to run specific tasks when the control receives focus.
...
One way to solve this problem is to use [`setTimeout()`](/content/Web/API/setTimeout) to delay the call to `nameEl.focus()` until the next event cycle, and give Svelte the opportunity to update the DOM.
...
2. Next, call `tick()` with [`await`](/content/Web/JavaScript/Reference/Operators/await) from an [async function](/content/Web/JavaScript/Reference/Statements/async_function); update `onEdit()` like so:
...
To select the text of a DOM input node, we have to call [`select()`](/content/Web/API/HTMLInputElement/select). To get this function called whenever the node gets focused, we need an event listener along these lines:

```js
node.addEventListener("focus", (event) => node.select());
```

And, in order to avoid memory leaks, we should also call the [`removeEventListener()`](/content/Web/API/EventTarget/removeEventListener) function when the node is destroyed.
