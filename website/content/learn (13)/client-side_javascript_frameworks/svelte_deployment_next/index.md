...
- [Svelte for VS Code](https://marketplace.visualstudio.com/items?itemName=svelte.svelte-vscode): The officially supported VS Code plugin for working with `.svelte` files, which we looked at in our [TypeScript article](/content/Learn/Tools_and_testing/Client-side_JavaScript_frameworks/Svelte_TypeScript).
