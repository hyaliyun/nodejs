...
7. If you try adding new to-dos at this point, the new to-dos are added to the to-dos array, but our UI is not updated. Remember that in Svelte [reactivity is triggered with assignments](https://svelte.dev/docs/svelte-components#script-2-assignments-are-reactive). That means that the `addTodo()` function is executed, the element is added to the `todos` array, but Svelte won't detect that the push method modified the array, so it won't refresh the tasks `<ul>`.
   ...
   Just adding `todos = todos` to the end of the `addTodo()` function would solve the problem, but it seems strange to have to include that at the end of the function. Instead, we'll take out the `push()` method and use [spread syntax](/content/Web/JavaScript/Reference/Operators/Spread_syntax) to achieve the same result: we'll assign a value to the `todos` array equal to the `todos` array plus the new object.

   > **Note:** `Array` has several mutable operations: [`push()`](/content/Web/JavaScript/Reference/Global_Objects/Array/push), [`pop()`](/content/Web/JavaScript/Reference/Global_Objects/Array/pop), [`splice()`](/content/Web/JavaScript/Reference/Global_Objects/Array/splice), [`shift()`](/content/Web/JavaScript/Reference/Global_Objects/Array/shift), [`unshift()`](/content/Web/JavaScript/Reference/Global_Objects/Array/unshift), [`reverse()`](/content/Web/JavaScript/Reference/Global_Objects/Array/reverse), and [`sort()`](/content/Web/JavaScript/Reference/Global_Objects/Array/sort). Using them often causes side effects and bugs that are hard to track. By using the spread syntax instead of `push()` we avoid mutating the array, which is considered a good practice.
