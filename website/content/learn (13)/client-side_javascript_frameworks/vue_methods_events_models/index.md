...
3. Let's add in an HTML form that lets you enter a new todo item and submit it into the app. We need a [`<form>`](/content/Web/HTML/Element/form) with a [`<label>`](/content/Web/HTML/Element/label), an [`<input>`](/content/Web/HTML/Element/input), and a [`<button>`](/content/Web/HTML/Element/button). Update your template as follows:
...
3. When you run this, the app still posts the data to the server, causing a refresh. Since we're doing all of our processing on the client, there's no server to handle the postback. We also lose all local state on page refresh. To prevent the browser from posting to the server, we need to stop the event's default action while bubbling up through the page ([`Event.preventDefault()`](/content/Web/API/Event/preventDefault), in vanilla JavaScript). Vue has a special syntax called **event modifiers** that can handle this for us right in our template.

   Modifiers are appended to the end of an event with a dot like so: `@event.modifier`. Here is a list of event modifiers:

   - `.stop`: Stops the event from propagating. Equivalent to [`Event.stopPropagation()`](/content/Web/API/Event/stopPropagation) in regular JavaScript events.
   - `.prevent`: Prevents the event's default behavior. Equivalent to [`Event.preventDefault()`](/content/Web/API/Event/preventDefault).
   - `.self`: Triggers the handler only if the event was dispatched from this exact element.
   - `{.key}`: Triggers the event handler only via the specified key. [MDN has a list of valid key values](/content/Web/API/UI_Events/Keyboard_event_key_values); multi-word keys just need to be converted to {{Glossary("kebab_case", "kebab-case")}} (e.g. `page-down`).
   ...
   - `.passive`: Equivalent to using the `{ passive: true }` parameter when creating an event listener in vanilla JavaScript using [`addEventListener()`](/content/Web/API/EventTarget/addEventListener).
...
If you try submitting the form now, you'll notice that the page doesn't reload. If you open the console, you can see the results of the [`console.log()`](/content/Web/API/console/log_static) we added inside our `onSubmit()` method.
