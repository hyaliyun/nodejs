...
This heading constant is known as a **JSX expression**. React can use it to render that [`<h1>`](/content/Web/HTML/Element/Heading_Elements) tag in our app.

Suppose we wanted to wrap our heading in a [`<header>`](/content/Web/HTML/Element/header) tag, for semantic reasons? The JSX approach allows us to nest our elements within each other, just like we do with HTML:
...
> ```
>
> However, this looks kind of awkward, because the [`<header>`](/content/Web/HTML/Element/header) tag that starts the expression is not indented to the same position as its corresponding closing tag.
...
It's possible to [add React to an existing project](https://react.dev/learn/add-react-to-an-existing-project) by copying some [`<script>`](/content/Web/HTML/Element/script) elements into an HTML file, but using Vite will allow you to spend more time building your app and less time fussing with setup.
...
The `App.jsx` file consists of three main parts: some [`import`](/content/Web/JavaScript/Reference/Statements/import) statements at the top, the `App()` function in the middle, and an [`export`](/content/Web/JavaScript/Reference/Statements/export) statement at the bottom. Most React components follow this pattern.
