...
We will also see how to develop our own custom store to persist the todo information to [web storage](/content/Web/API/Web_Storage_API), allowing our to-dos to persist over page reloads.
...
To learn more about detecting and fixing accessibility issues, check out our [Handling common accessibility problems](/content/Learn/Tools_and_testing/Cross_browser_testing/Accessibility) article.
...
Now we have to find a way to persist these to-dos. We could implement some code in our `App.svelte` component to read and save our to-dos to [web storage](/content/Web/API/Web_Storage_API) or to a web service.
...
2. Give it the following content:
   ...
   - We use the [`localStorage.getItem(key)`](/content/Web/API/Storage/getItem) and [`localStorage.setItem(key, value)`](/content/Web/API/Storage/setItem) methods to read and write information to web storage, and the [`toString()`](/content/Web/JavaScript/Reference/Global_Objects/Object/toString) and `toObj()` (which uses [`JSON.parse()`](/content/Web/JavaScript/Reference/Global_Objects/JSON/parse)) helper functions to convert the values.
