...
> **Note:** [`aria-label`](/content/Web/Accessibility/ARIA/Attributes/aria-label) provides a label for assistive technology to make use of — for example, for a screen reader to read out. This is useful in such cases where we have an [`<input>`](/content/Web/HTML/Element/input) being used with no corresponding HTML text that could be turned into a label.
