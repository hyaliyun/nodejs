...
You can write tests for CSS features by testing for the existence of _[element.style.property](/content/Web/API/HTMLElement/style)_ (e.g. `paragraph.style.rotate`) in JavaScript.

A classic example might be to test for [Subgrid](/content/Web/CSS/CSS_grid_layout/Subgrid) support in a browser; for browsers that support the `subgrid` value for a subgrid value for [`grid-template-columns`](/content/Web/CSS/grid-template-columns) and [`grid-template-rows`](/content/Web/CSS/grid-template-rows), we can use subgrid in our layout. For browsers that don't, we could use regular grid that works fine but is not as cool-looking.
...
In our conditional statement, we test to see if the {{cssxref("grid-template-columns")}} property supports the `subgrid` value using [`CSS.supports()`](/content/Web/API/CSS/supports_static).

#### @supports

CSS has a native feature detection mechanism: the {{cssxref("@supports")}} at-rule. This works in a similar manner to [media queries](/content/Web/CSS/CSS_media_queries) except that instead of selectively applying CSS depending on a media feature like a resolution, screen width or {{glossary("aspect ratio")}}, it selectively applies CSS depending on whether a CSS feature is supported, similar to `CSS.supports()`.
...
- Members of an object
  ...
  - : Check whether a particular method or property (typically an entry point into using the API or other feature you are detecting) exists in its parent `Object`.
    ...
    Our earlier example used this pattern to detect [Geolocation](/content/Web/API/Geolocation_API) support by testing the [`navigator`](/content/Web/API/Navigator) object for a `geolocation` member:
...
- Properties of an element
  ...
  - : Create an element in memory using {{domxref("Document.createElement()")}} and then check if a property exists on it.
    ...
    This example shows a way of detecting [Canvas API](/content/Web/API/Canvas_API) support:
