...
See the `{{htmlelement("select")}}`, `{{htmlelement("optgroup")}}` and `{{htmlelement("option")}}` elements and the [`size` attribute](/content/Web/HTML/Attributes/size).
...
See the `{{htmlelement("datalist")}}` and `` elements and the [`list`](/content/Web/HTML/Element/input#list) attribute.
