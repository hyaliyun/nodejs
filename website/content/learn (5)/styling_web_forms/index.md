...
In the previous few articles, we showed how to create web forms in HTML. Now, we'll show how to style them in [CSS](/content/Web/CSS).
...
3. Single-line text s (e.g. type text, url, email), except for [`<input type="search">`](/content/Web/HTML/Element/input/search).
4. Multi-line {{HTMLElement("textarea")}}
5. Buttons (both  and {{HTMLElement("button")}})
6. {{HTMLElement("label")}}
...
The article [Advanced form styling](/content/Learn/Forms/Advanced_form_styling) shows how to style these.
...
The articles [Advanced form styling](/content/Learn/Forms/Advanced_form_styling) and [How to build custom form controls](/content/Learn/Forms/How_to_build_custom_form_controls) describe how to style these.
...
The "easy-to-style" widgets in the previous section may be styled using techniques from the articles [Your first form](/content/Learn/Forms/Your_first_form) and [CSS building blocks](/content/Learn/CSS/Building_blocks). There are also special selectors — [UI pseudo-classes](/content/Learn/Forms/UI_pseudo-classes) — that enable styling based on the current state of the UI.
...
The HTML is only slightly more involved than the example we used in [the first article of this guide](/content/Learn/Forms/Your_first_form); it just has a few extra IDs and a heading.
...
Notice that we've used some [CSS grid](/content/Web/CSS/CSS_grid_layout) and [Flexbox](/content/Web/CSS/CSS_flexible_box_layout) to lay out the form. Using this we can easily position our elements, including the title and all the form elements:
...
The {{HTMLElement("button")}} element is really convenient to style with CSS; you can do whatever you want, even using [pseudo-elements](/content/Web/CSS/Pseudo-elements):
...
You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: Styling basics](/content/Learn/Forms/Test_your_skills:_Styling_basics).

## Summary

As you can see, as long as we want to build forms with just text fields and buttons, it's easy to style them using CSS. [In the next article](/content/Learn/Forms/Advanced_form_styling), we will see how to handle form widgets which fall into the "bad" and "ugly" categories.
