...
In this article, we will see what can be done with CSS to style the types of form control that are more difficult to style — the "bad" and "ugly" categories. As we saw [in the previous article](/content/Learn/Forms/Styling_web_forms), text fields and buttons are perfectly easy to style; now we will dig into styling the more problematic bits.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/content/Learn/CSS/First_steps">CSS</a>.
      </td>
    </tr>
    <tr>
...
- [`<input type="search">`](/content/Web/HTML/Element/input/search)
...
- [`<input type="color">`](/content/Web/HTML/Element/input/color)
- Date-related controls such as [`<input type="datetime-local">`](/content/Web/HTML/Element/input/datetime-local)
- [`<input type="range">`](/content/Web/HTML/Element/input/range)
- [`<input type="file">`](/content/Web/HTML/Element/input/file)
- {{HTMLElement("progress")}} and {{HTMLElement("meter")}}

Let's first talk about the [`appearance`](/content/Web/CSS/appearance) property, which is pretty useful for making all of the above more stylable.
...
[`<input type="search">`](/content/Web/HTML/Element/input/search) is basically just a text input, so why is `appearance: none;` useful here? The answer is that Safari search boxes have some styling restrictions — you can't adjust their `height` or `font-size` freely, for example.
...
You'll find out more about such pseudo-classes and more in the [next article](/content/Learn/Forms/UI_pseudo-classes); the above ones do the following:
...
Now let's turn our attention to the "ugly" controls — the ones that are really hard to thoroughly style. In short, these are drop-down boxes, complex control types like [`color`](/content/Web/HTML/Element/input/color) and [`datetime-local`](/content/Web/HTML/Element/input/datetime-local), and feedback—oriented controls like {{HTMLElement("progress")}} and {{HTMLElement("meter")}}.
...
>
> Also bear in mind that we've added some JavaScript to the page that lists the files selected by the file picker, below the control itself. This is a simplified version of the example found on the [`<input type="file">`](/content/Web/HTML/Element/input/file#examples) reference page.
...
We then created our own icon using generated content. We put an extra wrapper around the control, because [`::before`](/content/Web/CSS/::before)/[`::after`](/content/Web/CSS/::after) don't work on `<select>` elements (because their content is fully controlled by the browser):
...
The date/time input types ([`datetime-local`](/content/Web/HTML/Element/input/datetime-local), [`time`](/content/Web/HTML/Element/input/time), [`week`](/content/Web/HTML/Element/input/week), [`month`](/content/Web/HTML/Element/input/month)) all have the same major associated issue. The actual containing box is as easy to style as any text input, and what we've got in this demo looks fine.
...
> [!NOTE]
> It is worth mentioning [`<input type="number">`](/content/Web/HTML/Element/input/number) here too — this also has a spinner that you can use to increment/decrement values, so potentially suffers from the same problem. However, in the case of the `number` type the data being collected is simpler, and it is easy to just use a `tel` input type instead which has the appearance of `text`, but displays the numeric keypad in devices with touch keyboards.

### Range input types

[`<input type="range">`](/content/Web/HTML/Element/input/range) is annoying to style. You can use something like the following to remove the default slider track completely and replace it with a custom style (a thin red track, in this case):
...
The article [How to build custom form controls](/content/Learn/Forms/How_to_build_custom_form_controls) provides an example of how to build a custom designed select with HTML, CSS, and JavaScript.
...
In the next article of this module, we will explore the different [UI pseudo-classes](/content/Learn/Forms/UI_pseudo-classes) available to us in modern browsers for styling forms in different states.
