...
Forms allow users to enter data, which is generally sent to a web server for processing and storage (see [Sending form data](/content/Learn/Forms/Sending_and_retrieving_form_data) later in the module), or used on the client-side to immediately update the interface in some way (for example, add another item to a list, or show or hide a UI feature).
...
The controls can be single or multi-line text fields, dropdown boxes, buttons, checkboxes, or radio buttons, and are mostly created using the  element, although there are some other elements to learn about too.
...
Ok, let's have a go at creating the HTML for our form. We will use the following HTML elements: {{HTMLelement("form")}}, {{HTMLelement("label")}}, , {{HTMLelement("textarea")}}, and {{HTMLelement("button")}}.
...
This element formally defines a form. It's a container element like a {{HTMLelement("section")}} or {{HTMLelement("footer")}} element, but specifically for containing forms; it also supports some specific attributes to configure the way the form behaves. All of its attributes are optional, but it's standard practice to always set at least the [`action`](/content/Web/HTML/Element/form#action) and [`method`](/content/Web/HTML/Element/form#method) attributes:

- The `action` attribute defines the location (URL) where the form's collected data should be sent when it is submitted.
- The `method` attribute defines which HTTP method to send the data with (usually `get` or `post`).

> [!NOTE]
> We'll look at how those attributes work in our [Sending form data](/content/Learn/Forms/Sending_and_retrieving_form_data) article later on.
...
Note the use of the [`for`](/content/Web/HTML/Attributes/for) attribute on all {{HTMLelement("label")}} elements, which takes as its value the [`id`](/content/Web/HTML/Global_attributes/id) of the form control with which it is associated — this is how you associate a form control with its label.
...
You'll find further details of form labels in [How to structure a web form](/content/Learn/Forms/How_to_structure_a_web_form).
...
You'll find more about this in the [Basic native form controls](/content/Learn/Forms/Basic_native_form_controls) article later on.
...
- For the second input, we use the value {{HTMLelement("input/email", "email")}}, which defines a single-line text field that only accepts a well-formed email address.
  ...
  You'll find out more about form validation in the [client-side form validation](/content/Learn/Forms/Form_validation) article later on.
...
To define the default value of an  element you have to use the [`value`](/content/Web/HTML/Element/input#value) attribute like this:
...
> [!NOTE]
> You can also use the  element with the corresponding `type` to produce a button, for example `<input type="submit">`. The main advantage of the {{HTMLelement("button")}} element is that the  element only allows plain text in its label whereas the {{HTMLelement("button")}} element allows full HTML content, allowing more complex, creative button content.
...
The {{HTMLelement("form")}} element defines where and how to send the data thanks to the [`action`](/content/Web/HTML/Element/form#action) and [`method`](/content/Web/HTML/Element/form#method) attributes.
...
That data will be sent to the URL `/my-handling-form-page` using the [HTTP `POST`](/content/Web/HTTP/Methods/POST) method.
...
It's beyond the scope of this guide to go deeply into that subject, but if you want to know more, we have provided some examples in our [Sending form data](/content/Learn/Forms/Sending_and_retrieving_form_data) article later on.
