...
The {{htmlelement("video")}} and {{htmlelement("audio")}} elements allow us to embed video and audio into web pages. As we showed in [Video and audio content](/content/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content), a typical implementation looks like this:
...
You can review what all the HTML features do in the article linked above; for our purposes here, the most interesting attribute is [`controls`](/content/Web/HTML/Element/video#controls), which enables the default set of playback controls. If you don't specify this, you get no playback controls:
...
- We use the {{cssxref("content")}} property to set the content to be displayed in each case to be equal to the contents of the [`data-icon`](/content/Learn/HTML/Howto/Use_data_attributes) attribute. In the case of our play button, `data-icon` contains a capital "P".
...
1. Next, let's add functionality to handle stopping the video. Add the following [`addEventListener()`](/content/Web/API/EventTarget/addEventListener) lines below the previous one you added:
...
1. First of all, add the following two [`addEventListener()`](/content/Web/API/EventTarget/addEventListener) lines below the previous ones:
...
- [Video and audio content](/content/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content) — simple guide to `<video>` and `<audio>` HTML.
- [Audio and video delivery](/content/Web/Media/Audio_and_video_delivery) — detailed guide to delivering media inside the browser, with many tips, tricks, and links to further more advanced tutorials.
- [Audio and video manipulation](/content/Web/Media/Audio_and_video_manipulation) — detailed guide to manipulating audio and video, e.g. with [Canvas API](/content/Web/API/Canvas_API), [Web Audio API](/content/Web/API/Web_Audio_API), and more.
