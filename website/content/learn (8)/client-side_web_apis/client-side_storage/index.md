...
Elsewhere in the MDN learning area, we talked about the difference between [static sites](/content/Learn/Server-side/First_steps/Client-Server_overview#static_sites) and [dynamic sites](/content/Learn/Server-side/First_steps/Client-Server_overview#dynamic_sites). Most major modern websites are dynamic — they store data on the server using some kind of database (server-side storage), then run [server-side](/content/Learn/Server-side) code to retrieve needed data, insert it into static page templates, and serve the resulting HTML to the client to be displayed by the user's browser.
...
The [Web Storage API](/content/Web/API/Web_Storage_API) is very easy to use — you store simple name/value pairs of data (limited to strings, numbers, etc.) and retrieve these values when needed.
...
> [!NOTE]
> There is another, slightly more complex example to explore at [Using the Web Storage API](/content/Web/API/Web_Storage_API/Using_the_Web_Storage_API).
...
To learn more about the IndexedDB API, see [Using IndexedDB](/content/Web/API/IndexedDB_API/Using_IndexedDB).
...
2. Have a look at the files. You'll see that the HTML defines a website with a header and footer, as well as a main content area that contains a place to display notes, and a form for entering new notes into the database. The CSS provides some styling to make it clearer what is going on. The JavaScript file contains five declared constants containing references to the {{htmlelement("ul")}} element the notes will be displayed in, the title and body  elements, the {{htmlelement("form")}} itself, and the {{htmlelement("button")}}.
...
- The first part of this could use some explaining — we retrieve the ID of the record to be deleted using `Number(e.target.parentNode.getAttribute('data-note-id'))` — recall that the ID of the record was saved in a `data-note-id` attribute on the `<li>` when it was first displayed. We do however need to pass the attribute through the global built-in [`Number()`](/content/Web/JavaScript/Reference/Global_Objects/Number) object as it is of datatype string, and therefore wouldn't be recognized by the database, which expects a number.
...
This is where [Service workers](/content/Web/API/Service_Worker_API) and the closely-related [Cache API](/content/Web/API/Cache) come in.
...
If a match is found, we return it as the custom response. If not, we [fetch()](/content/Web/API/Window/fetch) the response from the network and return that instead.
...
- [Web storage API](/content/Web/API/Web_Storage_API)
- [IndexedDB API](/content/Web/API/IndexedDB_API)
- [Cookies](/content/Web/HTTP/Cookies)
- [Service worker API](/content/Web/API/Service_Worker_API)
