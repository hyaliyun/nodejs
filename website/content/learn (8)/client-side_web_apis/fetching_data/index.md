...
Another very common task in modern websites and applications is retrieving individual data items from the server to update sections of a webpage without having to load an entire new page. This seemingly small detail has had a huge impact on the performance and behavior of sites, so in this article, we'll explain the concept and look at technologies that make it possible: in particular, the [Fetch API](/content/Web/API/Fetch_API).
...
The main API here is the [Fetch API](/content/Web/API/Fetch_API). This enables JavaScript running in a page to make an [HTTP](/content/Web/HTTP) request to a server to retrieve specific resources. When the server provides them, the JavaScript can use the data to update the page, typically by using [DOM manipulation APIs](/content/Learn/JavaScript/Client-side_web_APIs/Manipulating_documents). The data requested is often [JSON](/content/Learn/JavaScript/Objects/JSON), which is a good format for transferring structured data, but can also be HTML or just text.
...
Modern browsers will not run HTTP requests if you just run the example from a local file. This is because of security restrictions (for more on web security, read [Website security](/content/Learn/Server-side/First_steps/Website_security)).

To get around this, we need to test the example by running it through a local web server. To find out how to do this, read [our guide to setting up a local testing server](/content/Learn/Common_questions/Tools_and_setup/set_up_a_local_testing_server).
...
- check that the server didn't return an error (such as [`404 Not Found`](/content/Web/HTTP/Status/404)). If it did, we throw the error.
- call {{domxref("Response.json","json()")}} on the response. This will retrieve the data as a [JSON object](/content/Learn/JavaScript/Objects/JSON). We return the promise returned by `response.json()`.
...
2. Call its [`open()`](/content/Web/API/XMLHttpRequest/open) method to initialize it.
3. Add an event listener to its [`load`](/content/Web/API/XMLHttpRequest/load_event) event, which fires when the response has completed successfully. In the listener we call `initialize()` with the data.
4. Add an event listener to its [`error`](/content/Web/API/XMLHttpRequest/error_event) event, which fires when the request encounters an error
5. Send the request.

We also have to wrap the whole thing in the [try...catch](/content/Web/JavaScript/Reference/Statements/try...catch) block, to handle any errors thrown by `open()` or `send()`.
...
- [Using Fetch](/content/Web/API/Fetch_API/Using_Fetch)
- [Promises](/content/Web/JavaScript/Reference/Global_Objects/Promise)
- [Working with JSON data](/content/Learn/JavaScript/Objects/JSON)
- [An overview of HTTP](/content/Web/HTTP/Overview)
- [Server-side website programming](/content/Learn/Server-side)
