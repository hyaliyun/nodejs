...
> 2. in that tab, open the JavaScript console in your [browser's developer tools](/content/Learn/Common_questions/Tools_and_setup/What_are_browser_developer_tools)
...
Note that `Started request…` is logged before we receive the response. Unlike a synchronous function, `fetch()` returns while the request is still going on, enabling our program to stay responsive. The response shows the `200` (OK) [status code](/content/Web/HTTP/Status), meaning that our request succeeded.
...
Also, note that you can only use `await` inside an `async` function, unless your code is in a [JavaScript module](/content/Web/JavaScript/Guide/Modules). That means you can't do this in a normal script:
...
Many modern Web APIs are promise-based, including [WebRTC](/content/Web/API/WebRTC_API), [Web Audio API](/content/Web/API/Web_Audio_API), [Media Capture and Streams API](/content/Web/API/Media_Capture_and_Streams_API), and many more.

## See also

- [`Promise()`](/content/Web/JavaScript/Reference/Global_Objects/Promise)
- [Using promises](/content/Web/JavaScript/Guide/Using_promises)
