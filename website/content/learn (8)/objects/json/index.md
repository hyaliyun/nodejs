...
<table>
  <tbody>
    <tr>
      ...
      <td>
        A basic understanding of HTML and CSS, familiarity with JavaScript basics (see <a href="/content/Learn/JavaScript/First_steps">First steps</a> and <a href="/content/Learn/JavaScript/Building_blocks">Building blocks</a>) and OOJS basics (see <a href="/content/Learn/JavaScript/Objects/Basics">Introduction to objects</a>).
      </td>
    </tr>
    <tr>
...
This is not a big issue — JavaScript provides a global [JSON](/content/Web/JavaScript/Reference/Global_Objects/JSON) object that has methods available for converting between the two.
...
If we loaded this string into a JavaScript program and parsed it into a variable called `superHeroes` for example, we could then access the data inside it using the same dot/bracket notation we looked at in the [JavaScript object basics](/content/Learn/JavaScript/Objects/Basics) article.
For example:
...
To obtain the JSON, we use an API called [Fetch](/content/Web/API/Fetch_API).
...
Here we first create an {{HTMLElement("Heading_Elements", "h1")}} element with [`createElement()`](/content/Web/API/Document/createElement), set its [`textContent`](/content/Web/API/Node/textContent) to equal the `squadName` property of the object, then append it to the header using [`appendChild()`](/content/Web/API/Node/appendChild). We then do a very similar operation with a paragraph: create it, set its text content and append it to the header. The only difference is that its text is set to a [template literal](/content/Web/JavaScript/Reference/Template_literals) containing both the `homeTown` and `formed` properties of the object.
...
Next, we use a [for...of loop](/content/Learn/JavaScript/Building_blocks/Looping_code#the_for...of_loop) to loop through each object in the array. For each one, we:
...
> You should also refer back to our [JavaScript object basics](/content/Learn/JavaScript/Objects/Basics) article for more information on dot and bracket notation.
...
But sometimes we aren't so lucky — sometimes we receive a raw JSON string, and we need to convert it to an object ourselves. And when we want to send a JavaScript object across the network, we need to convert it to JSON (a string) before sending it. Luckily, these two problems are so common in web development that a built-in [JSON](/content/Web/JavaScript/Reference/Global_Objects/JSON) object is available in browsers, which contains the following two methods:

- [`parse()`](/content/Web/JavaScript/Reference/Global_Objects/JSON/parse): Accepts a JSON string as a parameter, and returns the corresponding JavaScript object.
- [`stringify()`](/content/Web/JavaScript/Reference/Global_Objects/JSON/stringify): Accepts an object as a parameter, and returns the equivalent JSON string.
...
You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: JSON](/content/Learn/JavaScript/Objects/Test_your_skills:_JSON).
...
- [JSON reference](/content/Web/JavaScript/Reference/Global_Objects/JSON)
- [Fetch API overview](/content/Web/API/Fetch_API)
- [Using Fetch](/content/Web/API/Fetch_API/Using_Fetch)
- [HTTP request methods](/content/Web/HTTP/Methods)
