  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/content/Learn/JavaScript/First_steps">First steps</a> and
        <a href="/content/Learn/JavaScript/Building_blocks"
          >Building blocks</a
        >) and OOJS basics (see
        <a href="/content/Learn/JavaScript/Objects/Basics"
...
The `random()` function takes two numbers as arguments, and returns a random number in the range between the two. The `randomRGB()` function generates a random color represented as an [`rgb()`](/content/Web/CSS/color_value/rgb) string.
...
- First, we use [`beginPath()`](/content/Web/API/CanvasRenderingContext2D/beginPath) to state that we want to draw a shape on the paper.
- Next, we use [`fillStyle`](/content/Web/API/CanvasRenderingContext2D/fillStyle) to define what color we want the shape to be — we set it to our ball's `color` property.
- Next, we use the [`arc()`](/content/Web/API/CanvasRenderingContext2D/arc) method to trace an arc shape on the paper. Its parameters are:
...
- Last of all, we use the [`fill()`](/content/Web/API/CanvasRenderingContext2D/fill) method, which basically states "finish drawing the path we started with `beginPath()`, and fill the area it takes up with the color we specified earlier in `fillStyle`."
...
- We then use a common algorithm to check the collision of two circles. We are basically checking whether any of the two circle's areas overlap. This is explained further in [2D collision detection](/content/Games/Techniques/2D_collision_detection).
...
- [Canvas tutorial](/content/Web/API/Canvas_API/Tutorial) — a beginner's guide to using 2D canvas.
- [requestAnimationFrame()](/content/Web/API/Window/requestAnimationFrame)
- [2D collision detection](/content/Games/Techniques/2D_collision_detection)
- [3D collision detection](/content/Games/Techniques/3D_collision_detection)
- [2D breakout game using pure JavaScript](/content/Games/Tutorials/2D_Breakout_game_pure_JavaScript) — a great beginner's tutorial showing how to build a 2D game.
- [2D breakout game using Phaser](/content/Games/Tutorials/2D_breakout_game_Phaser) — explains the basics of building a 2D game using a JavaScript game library.
