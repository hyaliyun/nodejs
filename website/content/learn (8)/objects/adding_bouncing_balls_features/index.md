...
This adds a `keydown` event listener to the `window` object so that when a key is pressed, the event object's [`key`](/content/Web/API/KeyboardEvent/key) property is consulted to see which key is pressed. If it is one of the four specified keys, then the evil circle will move left/right/up/down.
...
- We want the evil circle to not be filled in, but rather just have an outer line (stroke). You can achieve this by updating [`fillStyle`](/content/Web/API/CanvasRenderingContext2D/fillStyle) and [`fill()`](/content/Web/API/CanvasRenderingContext2D/fill) to [`strokeStyle`](/content/Web/API/CanvasRenderingContext2D/strokeStyle) and [`stroke()`](/content/Web/API/CanvasRenderingContext2D/stroke) respectively.
- We also want to make the stroke a bit thicker, so you can see the evil circle a bit more easily. This can be achieved by setting a value for [`lineWidth`](/content/Web/API/CanvasRenderingContext2D/lineWidth) somewhere after the [`beginPath()`](/content/Web/API/CanvasRenderingContext2D/beginPath) call (3 will do).
