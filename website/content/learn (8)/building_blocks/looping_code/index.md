...
One type of collection is the {{jsxref("Array")}}, which we met in the [Arrays](/content/Learn/JavaScript/First_steps/Arrays) chapter of this course.
...
This looks a lot like `map()`, except the function we pass in returns a [boolean](/content/Learn/JavaScript/First_steps/Variables#booleans): if it returns `true`, then the item is included in the new array.
...
Note that `map()` and `filter()` are both often used with _function expressions_, which we will learn about in the [Functions](/content/Learn/JavaScript/Building_blocks/Functions) module.
...
If you want to exit a loop before all the iterations have been completed, you can use the [break](/content/Web/JavaScript/Reference/Statements/break) statement.
We already met this in the previous article when we looked at [switch statements](/content/Learn/JavaScript/Building_blocks/conditionals#switch_statements) — when a case is met in a switch statement that matches the input expression, the `break` statement immediately exits the switch statement and moves on to the code after it.
...
First, some simple HTML — a text  allowing us to enter a name to search for, a {{htmlelement("button")}} element to submit a search, and a {{htmlelement("p")}} element to display the results in:
...
3. We store the value entered into the text input in a variable called `searchName`, before then emptying the text input and focusing it again, ready for the next search.
   Note that we also run the [`toLowerCase()`](/content/Web/JavaScript/Reference/Global_Objects/String/toLowerCase) method on the string, so that searches will be case-insensitive.
4. Now on to the interesting part, the `for...of` loop:
   ...
   2. We then use a conditional statement to test whether `splitContact[0]` (the contact's name, again lower-cased with [`toLowerCase()`](/content/Web/JavaScript/Reference/Global_Objects/String/toLowerCase)) is equal to the inputted `searchName`.
...
The [continue](/content/Web/JavaScript/Reference/Statements/continue) statement works similarly to `break`, but instead of breaking out of the loop entirely, it skips to the next iteration of the loop.
...
2. Inside the loop, we find the square root of each number using [Math.sqrt(i)](/content/Web/JavaScript/Reference/Global_Objects/Math/sqrt), then check whether the square root is an integer by testing whether it is the same as itself when it has been rounded down to the nearest integer (this is what [Math.floor()](/content/Web/JavaScript/Reference/Global_Objects/Math/floor) does to the number it is passed).
...
First, let's have a look at the [while](/content/Web/JavaScript/Reference/Statements/while) loop. This loop's syntax looks like so:
...
The [do...while](/content/Web/JavaScript/Reference/Statements/do...while) loop is very similar, but provides a variation on the while structure:
...
Have a look at the [Useful string methods](/content/Learn/JavaScript/First_steps/Useful_string_methods) article for help.
...
> [!NOTE]
> There are other loop types/features too, which are useful in advanced/specialized situations and beyond the scope of this article. If you want to go further with your loop learning, read our advanced [Loops and iteration guide](/content/Web/JavaScript/Guide/Loops_and_iteration).

## Test your skills!

You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: Loops](/content/Learn/JavaScript/Building_blocks/Test_your_skills:_Loops).
...
If there is anything you didn't understand, feel free to read through the article again, or [contact us](/content/Learn#contact_us) to ask for help.

## See also

- [Loops and iteration in detail](/content/Web/JavaScript/Guide/Loops_and_iteration)
- [for...of reference](/content/Web/JavaScript/Reference/Statements/for...of)
- [for statement reference](/content/Web/JavaScript/Reference/Statements/for)
- [while](/content/Web/JavaScript/Reference/Statements/while) and [do...while](/content/Web/JavaScript/Reference/Statements/do...while) references
- [break](/content/Web/JavaScript/Reference/Statements/break) and [continue](/content/Web/JavaScript/Reference/Statements/continue) references
