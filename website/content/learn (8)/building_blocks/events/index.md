...
You can gather from this (and from glancing at the MDN [event reference](/content/Web/Events)) that there are **a lot** of events that can be fired.
...
- the string `"click"`, to indicate that we want to listen to the click event. Buttons can fire lots of other events, such as [`"mouseover"`](/content/Web/API/Element/mouseover_event) when the user moves their mouse over the button, or [`"keydown"`](/content/Web/API/Element/keydown_event) when the user presses a key and the button is focused.
- a function to call when the event happens. In our case, the function generates a random RGB color and sets the [`background-color`](/content/Web/CSS/background-color) of the page [`<body>`](/content/Web/HTML/Element/body) to that color.
...
- [`focus`](/content/Web/API/Element/focus_event) and [`blur`](/content/Web/API/Element/blur_event) — The color changes when the button is focused and unfocused; try pressing the tab to focus on the button and press the tab again to focus away from the button.
...
- [`dblclick`](/content/Web/API/Element/dblclick_event) — The color changes only when the button is double-clicked.
- [`mouseover`](/content/Web/API/Element/mouseover_event) and [`mouseout`](/content/Web/API/Element/mouseout_event) — The color changes when the mouse pointer hovers over the button, or when the pointer moves off the button, respectively.

Some events, such as `click`, are available on nearly any element. Others are more specific and only useful in certain situations: for example, the [`play`](/content/Web/API/HTMLMediaElement/play_event) event is only available on some elements, such as {{htmlelement("video")}}.

### Removing listeners

If you've added an event handler using `addEventListener()`, you can remove it again using the [`removeEventListener()`](/content/Web/API/EventTarget/removeEventListener) method. For example, this would remove the `changeBackground()` event handler:
...
These are a little out of scope for this article, but if you want to read them, visit the [`addEventListener()`](/content/Web/API/EventTarget/addEventListener) and [`removeEventListener()`](/content/Web/API/EventTarget/removeEventListener) reference pages.
...
The earliest method of registering event handlers found on the Web involved [_event handler HTML attributes_](/content/Web/HTML/Attributes#event_handler_attributes) (or _inline event handlers_) like the one shown above — the attribute value is literally the JavaScript code you want to run when the event occurs.
...
Now some JavaScript — here we implement a very simple check inside a handler for the [`submit`](/content/Web/API/HTMLFormElement/submit_event) event (the submit event is fired on a form when it is submitted) that tests whether the text fields are empty.
If they are, we call the [`preventDefault()`](/content/Web/API/Event/preventDefault) function on the event object — which stops the form submission — and then display an error message in the paragraph below our form to tell the user what's wrong:
...
For example, [Node.js](/content/Learn/Server-side/Express_Nodejs) is a very popular JavaScript runtime that enables developers to use JavaScript to build network and server-side applications.
...
You can also use JavaScript to build cross-browser add-ons — browser functionality enhancements — using a technology called [WebExtensions](/content/Mozilla/Add-ons/WebExtensions).
...
See the [`runtime.onMessage`](/content/Mozilla/Add-ons/WebExtensions/API/runtime/onMessage#examples) page for an example.
