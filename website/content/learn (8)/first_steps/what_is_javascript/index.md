...
It is the third layer of the layer cake of standard web technologies, two of which ([HTML](/content/Learn/HTML) and [CSS](/content/Learn/CSS)) we have covered in much more detail in other parts of the Learning Area.
...
- The [DOM (Document Object Model) API](/content/Web/API/Document_Object_Model) allows you to manipulate HTML and CSS, creating, removing and changing HTML, dynamically applying new styles to your page, etc.
...
- The [Geolocation API](/content/Web/API/Geolocation_API) retrieves geographical information.
  This is how [Google Maps](https://www.google.com/maps) is able to find your location and plot it on a map.
- The [Canvas](/content/Web/API/Canvas_API) and [WebGL](/content/Web/API/WebGL_API) APIs allow you to create animated 2D and 3D graphics.
...
> You will need to consider [cross browser testing](/content/Learn/Tools_and_testing/Cross_browser_testing) in more detail when you get closer to delivering production code (i.e. real code that real customers will use).
...
> [!NOTE]
> These APIs are advanced, and we'll not be covering any of these in this module. You can find out much more about these in our [Client-side web APIs module](/content/Learn/JavaScript/Client-side_web_APIs).
...
Let's briefly recap the story of what happens when you load a web page in a browser (first talked about in our [How CSS works](/content/Learn/CSS/First_steps/How_CSS_works#how_does_css_actually_work) article). When you load a web page in your browser, you are running your code (the HTML, CSS, and JavaScript) inside an execution environment (the browser tab). This is like a factory that takes in raw materials (the code) and outputs a product (the web page).
...
If you swapped the order of the first two lines of code, it would no longer work — instead, you'd get an error returned in the [browser developer console](/content/Learn/Common_questions/Tools_and_setup/What_are_browser_developer_tools) — `Uncaught ReferenceError: Cannot access 'button' before initialization`.
...
JavaScript can also be used as a server-side language, for example in the popular Node.js environment — you can find out more about server-side JavaScript in our [Dynamic Websites – Server-side programming](/content/Learn/Server-side) topic.
...
4. Save and refresh your browser. You'll discover that clicking the button has no effect, and if you check your browser's console, you'll see an error along the lines of `Cross-origin request blocked`. That's because like many external resources, JavaScript modules need to be loaded from the [same origin](/content/Web/Security/Same-origin_policy) as the HTML, and `file://` URLs don't qualify. There are two solutions to fix this problem:
   - Our recommended solution is to follow the [guide to set up a local testing server](/content/Learn/Common_questions/Tools_and_setup/set_up_a_local_testing_server). With the server program running and serving the `apply-javascript-external.html` and `script.js` files on port `8000`, open your browser and go to `http://localhost:8000`.
...
If you are using JavaScript to manipulate elements on the page (or more accurately, the [Document Object Model](/content/Learn/JavaScript/Client-side_web_APIs/Manipulating_documents#the_document_object_model)), your code won't work if the JavaScript is loaded and parsed before the HTML you are trying to do something to.
...
- If you still want to use non-module scripts in the document head, which could block the whole page from displaying, and could cause errors because it executes before the HTML is parsed:
  ...
  - For internal scripts, you should wrap the code in a [`DOMContentLoaded` event listener](/content/Web/API/Document/DOMContentLoaded_event).
...
In the next article, we will [plunge straight into the practical](/content/Learn/JavaScript/First_steps/A_first_splash), getting you to jump straight in and build your own JavaScript examples.
