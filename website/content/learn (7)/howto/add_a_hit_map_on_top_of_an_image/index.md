...
<caption>Here are some things you need to know</caption>
  <tbody>
    <tr>
      ...
      <td>
        You should already know how to <a href="/content/Learn/Getting_started_with_the_web">create a basic HTML document</a> and how to <a href="/content/Learn/HTML/Multimedia_and_embedding/Images_in_HTML#how_do_we_put_an_image_on_a_webpage">add accessible images to a webpage.</a>
      </td>
    </tr>
    <tr>
...
[Text links](/content/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks) (perhaps styled with CSS) are preferable to image maps for several reasons: text links are lightweight, maintainable, often more SEO-friendly, and support accessibility needs (e.g., screen readers, text-only browsers, translation services).
...
You insert your image [much the same way as always](/content/Learn/HTML/Multimedia_and_embedding/Images_in_HTML#how_do_we_put_an_image_on_a_webpage) (with an {{htmlelement("img")}} element and [`alt`](/content/Web/HTML/Element/img#alt) text). If the image is only present as a navigation device, you may write `alt=""`, provided you furnish appropriate [`alt`](/content/Web/HTML/Element/area#alt) text in the {{htmlelement('area')}} elements later on.

You will need a special [`usemap`](/content/Web/HTML/Element/img#usemap) attribute. Come up with a unique name, containing no spaces, for your image map. Then assign that name (preceded by a hash) as the value for the `usemap` attribute:
...
In this step, put all your code inside a {{htmlelement('map')}} element. `<map>` only needs one attribute, the same map [`name`](/content/Web/HTML/Element/map#name) as you used in your `usemap` attribute above:
...
- [`shape`](/content/Web/HTML/Element/area#shape)
...
- [`coords`](/content/Web/HTML/Element/area#coords)
...
- [`href`](/content/Web/HTML/Element/area#href)
...
- [`alt`](/content/Web/HTML/Element/area#alt)

  - : A mandatory attribute, telling people where the link goes or what it does. `alt` text only displays when the image is unavailable. Please refer to our [guidelines for writing accessible link text.](/content/Learn/HTML/Introduction_to_HTML/Creating_hyperlinks#use_clear_link_wording)
