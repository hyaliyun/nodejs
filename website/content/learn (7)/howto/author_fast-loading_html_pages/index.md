...
Reducing the number of files referenced in a web page lowers the number of [HTTP](/content/Web/HTTP) connections required to download a page, thereby reducing the time for these requests to be sent, and for their responses to be received.

Depending on a browser's cache settings, it may send a request with the [`If-Modified-Since`](/content/Web/HTTP/Headers/If-Modified-Since) header for each referenced file, asking whether the file has been modified since the last time it was downloaded. Too much time spent querying the last modified time of the referenced files can delay the initial display of the web page, since the browser must check the modification time for each of these files, before rendering the page.
...
Inline scripts can be expensive for page loading since the parser must assume that an inline script could modify the page structure while parsing is in progress. Reducing the use of inline scripts in general, and reducing the use of `document.write()` to output content in particular, can improve overall page loading. Use [DOM APIs to manipulate page content](/content/Learn/JavaScript/Client-side_web_APIs/Manipulating_documents), rather than `document.write()`.
...
Tables for layouts are a legacy method that should not be used anymore. Layouts utilizing [floats](/content/Learn/CSS/CSS_layout/Floats), [positioning](/content/Learn/CSS/CSS_layout/Positioning), [flexbox](/content/Learn/CSS/CSS_layout/Flexbox), or [grids](/content/Learn/CSS/CSS_layout/Grids) should be used instead.
...
and should specify widths of columns using the [`<col>`](/content/Web/HTML/Element/col) and the [`<colgroup>`](/content/Web/HTML/Element/colgroup) elements.

### Use lazy loading for images

By default, images are loaded **eagerly**; that is, the image is fetched and rendered as soon as it's processed in the HTML. All eagerly loaded images are rendered before the window's [`load`](/content/Web/API/Window/load_event) event is sent. Switching to lazy loading of images tells the browser to hold off on loading images until they're about to be needed to draw the {{Glossary("visual viewport")}}.

To mark an image for lazy loading, specify its [`loading`](/content/Web/HTML/Element/img#loading) attribute with a value of `lazy`. With this set, the image will only be loaded when it's needed.
...
Make the JavaScript scripts such that they are compatible with both the [async](/content/Web/HTML/Element/script#attributes) and the [defer](/content/Web/HTML/Element/script#attributes) attributes, and use [async](/content/Web/HTML/Element/script#attributes) whenever possible, especially if you have multiple script elements.
