  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/content/Learn/HTML/Introduction_to_HTML">basics of HTML</a>
        and how to
    ...
    <tr>
      ...
      <td>
        Learn how to use features like
         <a href="/content/Web/HTML/Element/img#srcset"><code>srcset</code></a> and the
...
> [!NOTE]
> The new features discussed in this article — [`srcset`](/content/Web/HTML/Element/img#srcset)/[`sizes`](/content/Web/HTML/Element/img#sizes)/{{htmlelement("picture")}} — are all supported in modern desktop and mobile browsers.
...
2. A space
3. The image's **intrinsic width in pixels** (`480w`) — note that this uses the `w` unit, not `px` as you might expect. An image's [intrinsic size](/content/Glossary/Intrinsic_Size) is its real size, which can be found by inspecting the image file on your computer (for example, on a Mac you can select the image in Finder and press
...
1. A **media condition** (`(max-width:600px)`) — you'll learn more about these in the [CSS topic](/content/Learn/CSS), but for now let's just say that a media condition describes a possible state that the screen can be in. In this case, we are saying "when the viewport width is 600 pixels or less".
2. A space
3. The **width of the slot** the image will fill when the media condition is true (`480px`)

> [!NOTE]
> In `sizes`, you can use any [length value](/content/Web/CSS/length). For example, rather than providing an absolute width (for example, `480px`), you can alternatively provide a width relative to the viewport (for example, `50vw`). However, you cannot use a percentage as the slot width. You may have noticed that the last slot width has no media condition (this is the default that is chosen when none of the media conditions are true). The browser ignores everything after the first matching condition, so be careful how you order the media conditions.
...
Older browsers that don't support these features will just ignore them. Instead, those browsers will go ahead and load the image referenced in the [`src`](/content/Web/HTML/Element/img#src) attribute as normal.
...
- **Resolution switching**: The problem whereby you want to serve smaller image files to narrow-screen devices, as they don't need huge images like desktop displays do — and to serve different resolution images to high density/low density screens. You can solve this problem using [vector graphics](/content/Learn/HTML/Multimedia_and_embedding/Adding_vector_graphics_to_the_Web) (SVG images) and the [`srcset`](/content/Web/HTML/Element/img#srcset) with [`sizes`](/content/Web/HTML/Element/img#sizes) attributes.

This also draws to a close the entire [Multimedia and embedding](/content/Learn/HTML/Multimedia_and_embedding) module! The only thing to do now before moving on is to try our [Multimedia and embedding assessment](/content/Learn/HTML/Multimedia_and_embedding/Mozilla_splash_page), and see how you get on. Have fun!
