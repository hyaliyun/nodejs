  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/content/Learn/HTML/Introduction_to_HTML/Getting_started"
          >Getting started with HTML</a
        >) and
        <a href="/content/Learn/HTML/Multimedia_and_embedding/Images_in_HTML"
          >Images in HTML</a
        >.
      </td>
    </tr>
...
- [`src`](/content/Web/HTML/Element/video#src)
...
Due to the intricacies of ensuring your app's media is viewable across every combination of browsers, platforms, and devices you wish to reach, choosing the best combination of codecs and container can be a complicated task. See [Choosing the right container](/content/Web/Media/Formats/Containers#choosing_the_right_container) for help selecting the container file format best suited for your needs; similarly, see [Choosing a video codec](/content/Web/Media/Formats/Video_codecs#choosing_a_video_codec) and [Choosing an audio codec](/content/Web/Media/Formats/Audio_codecs#choosing_an_audio_codec) for help selecting the first media codecs to use for your content and your target audience.
...
Each `<source>` element also has a [`type`](/content/Web/HTML/Element/source#type) attribute. This is optional, but it is advised that you include it. The `type` attribute contains the {{glossary("MIME type")}} of the file specified by the `<source>`, and browsers can use the `type` to immediately skip videos they don't understand. If `type` isn't included, browsers will load and try to play each file until they find one that works, which obviously takes time and is an unnecessary use of resources.

Refer to our [guide to media types and formats](/content/Web/Media/Formats) for help selecting the best containers and codecs for your needs, as well as to look up the right MIME types to specify for each.
...
- [`width`](/content/Web/HTML/Element/video#width) and [`height`](/content/Web/HTML/Element/video#height)
...
- [`autoplay`](/content/Web/HTML/Element/video#autoplay)
...
- [`loop`](/content/Web/HTML/Element/video#loop)
...
- [`muted`](/content/Web/HTML/Element/video#muted)
  - : Causes the media to play with the sound turned off by default.
- [`poster`](/content/Web/HTML/Element/video#poster)
...
- [`preload`](/content/Web/HTML/Element/video#preload)
...
Wouldn't it be nice to be able to provide these people with a transcript of the words being spoken in the audio/video? Well, thanks to HTML video, you can. To do so we use the [WebVTT](/content/Web/API/WebVTT_API) file format and the {{htmlelement("track")}} element.
...
2. Link to the `.vtt` file with the {{htmlelement("track")}} element. `<track>` should be placed within `<audio>` or `<video>`, but after all `<source>` elements. Use the [`kind`](/content/Web/HTML/Element/track#kind) attribute to specify whether the cues are `subtitles`, `captions`, or `descriptions`. Further, use [`srclang`](/content/Web/HTML/Element/track#srclang) to tell the browser what language you have written the subtitles in. Finally, add [`label`](/content/Web/HTML/Element/track#label) to help readers identify the language they are searching for.
...
In order to try this you need to host the files on a [local HTTP server](/content/Learn/Common_questions/Tools_and_setup/set_up_a_local_testing_server). In the output in browser, you'll see a video that has subtitles displayed, kind of like this:
...
4. Give both of them {{HTMLElement("source")}} elements so that browsers will find the audio format they support best and load it. These should include [`type`](/content/Web/HTML/Element/source#type) attributes.
...
You've reached the end of this article, but can you remember the most important information? You can find some further tests to verify that you've retained this information before you move on — see [Test your skills: Multimedia and embedding](/content/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content/Test_your_skills:_Multimedia_and_embedding). Note that the third assessment question in this test assumes knowledge of some of the techniques covered in the [next article](/content/Learn/HTML/Multimedia_and_embedding/Other_embedding_technologies), so you may want to read that before attempting it.

## Summary

And that's a wrap — we hope you had fun playing with video and audio in web pages! In the next article, we'll look at [other ways of embedding content](/content/Learn/HTML/Multimedia_and_embedding/Other_embedding_technologies) on the Web, using technologies like {{htmlelement("iframe")}} and {{htmlelement("object")}}.

## See also

- The HTML media elements: {{htmlelement("audio")}}, {{htmlelement("video")}}, {{htmlelement("source")}}, and {{htmlelement("track")}}
- [Adding captions and subtitles to video](/content/Web/Media/Audio_and_video_delivery/Adding_captions_and_subtitles_to_HTML5_video)
- [Audio and Video delivery](/content/Web/Media/Audio_and_video_delivery): A LOT of detail about putting audio and video onto web pages using HTML and JavaScript.
- [Audio and Video manipulation](/content/Web/Media/Audio_and_video_manipulation): A LOT of detail about manipulating audio and video using JavaScript (for example adding filters.)
- [Web media technologies](/content/Web/Media)
- [Guide to media types and formats on the web](/content/Web/Media/Formats)
- [Event reference > Media](/content/Web/Events#media)
