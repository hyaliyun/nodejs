...
<body>
  <iframe
    src="https://developer.mozilla.org/content/Glossary"
    width="100%"
    height="500"
    allowfullscreen
    sandbox>
...
- [`border: none`](/content/Web/CSS/border)
...
- [`allowfullscreen`](/content/Web/HTML/Element/iframe#allowfullscreen)
  - : If set, the `<iframe>` is able to be placed in fullscreen mode using the [Fullscreen API](/content/Web/API/Fullscreen_API) (somewhat beyond the scope of this article).
- [`src`](/content/Web/HTML/Element/iframe#src)
...
- [`width`](/content/Web/HTML/Element/iframe#width) and [`height`](/content/Web/HTML/Element/iframe#height)
  - : These attributes specify the width and height you want the iframe to be.
- [`sandbox`](/content/Web/HTML/Element/iframe#sandbox)
...
> **Note:** [Clickjacking](/content/Glossary/Clickjacking) is one kind of common iframe attack where hackers embed an invisible iframe into your document (or embed your document into their own malicious website) and use it to capture users' interactions. This is a common way to mislead users or steal sensitive data.

A quick example first though — try loading the previous example we showed above into your browser — you can [find it live on GitHub](https://mdn.github.io/learning-area/html/multimedia-and-embedding/other-embedding-technologies/iframe-detail.html) ([see the source code](https://github.com/mdn/learning-area/blob/main/html/multimedia-and-embedding/other-embedding-technologies/iframe-detail.html) too.) Instead of the page you expected, you'll probably see some kind of message to the effect of "I can't open this page", and if you look at the _Console_ in the [browser developer tools](/content/Learn/Common_questions/Tools_and_setup/What_are_browser_developer_tools), you'll see a message telling you why. In Firefox, you'll get told something like _The loading of "https\://developer.mozilla.org/content/Glossary" in a frame is denied by "X-Frame-Options" directive set to "DENY"_. This is because the developers that built MDN have included a setting on the server that serves the website pages to disallow them from being embedded inside `<iframe>`s (see [Configure CSP directives](#configure_csp_directives), below.) This makes sense — an entire MDN page doesn't really make sense to be embedded in other pages unless you want to do something like embed them on your site and claim them as your own — or attempt to steal data via [clickjacking](/content/Glossary/Clickjacking), which are both really bad things to do. Plus if everybody started to do this, all the additional bandwidth would start to cost Mozilla a lot of money.
...
If the content is licensed, you must obey the license terms. For example, the content on MDN is [licensed under CC-BY-SA](/content/MDN/Writing_guidelines/Attrib_copyright_license#documentation). That means, you must [credit us properly](https://wiki.creativecommons.org/wiki/Best_practices_for_attribution) when you quote our content, even if you make substantial changes.
...
> **Note:** [GitHub pages](/content/Learn/Common_questions/Tools_and_setup/Using_GitHub_pages) allow content to be served via HTTPS by default.
...
If absolutely required, you can add permissions back one by one (inside the `sandbox=""` attribute value) — see the [`sandbox`](/content/Web/HTML/Element/iframe#sandbox) reference entry for all the available options. One important note is that you should _never_ add both `allow-scripts` and `allow-same-origin` to your `sandbox` attribute — in that case, the embedded content could bypass the [Same-origin policy](/content/Glossary/Same-origin_policy) that stops sites from executing scripts, and use JavaScript to turn off sandboxing altogether.
...
{{Glossary("CSP")}} stands for **[content security policy](/content/Web/HTTP/CSP)** and provides [a set of HTTP Headers](/content/Web/HTTP/Headers/Content-Security-Policy) (metadata sent along with your web pages when they are served from a web server) designed to improve the security of your HTML document. When it comes to securing `<iframe>`s, you can _[configure your server to send an appropriate `X-Frame-Options` header.](/content/Web/HTTP/Headers/X-Frame-Options)_ This can prevent other websites from embedding your content in their web pages (which would enable [clickjacking](/content/Glossary/Clickjacking) and a host of other attacks), which is exactly what the MDN developers have done, as we saw earlier on.
...
<table class="standard-table no-markdown">
  ...
  <tbody>
    <tr>
      ...
      <td><a href="/content/Web/HTML/Element/embed#src"><code>src</code></a></td>
      <td><a href="/content/Web/HTML/Element/object#data"><code>data</code></a></td>
    </tr>
    <tr>
      <td>
      ...
      <td><a href="/content/Web/HTML/Element/embed#type"><code>type</code></a></td>
      <td><a href="/content/Web/HTML/Element/object#type"><code>type</code></a></td>
    </tr>
    <tr>
      <td>
      ...
      <td>
         <a href="/content/Web/HTML/Element/embed#height"><code>height</code></a><br /><a href="/content/Web/HTML/Element/embed#width"><code>width</code></a>
      </td>
      <td>
         <a href="/content/Web/HTML/Element/object#height"><code>height</code></a><br /><a href="/content/Web/HTML/Element/object#width"><code>width</code></a>
      </td>
    </tr>
    <tr>
...
There are many other technologies that involve embedding external content besides the ones we discussed here. We saw some in earlier articles, such as {{htmlelement("video")}}, {{htmlelement("audio")}}, and {{htmlelement("img")}}, but there are others to discover, such as {{htmlelement("canvas")}} for JavaScript-generated 2D and 3D graphics, and {{SVGElement("svg")}} for embedding vector graphics. We'll look at [SVG](/content/Web/SVG) in the next article of the module.
